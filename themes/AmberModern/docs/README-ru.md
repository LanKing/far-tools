🇺🇸 [English](../README.md) | 🇺🇦 [Українська](README-uk.md) | 🇧🇾 [Беларуская](README-be.md) | 🇵🇱 [Polski](README-pl.md) | 🇨🇿 [Čeština](README-cs.md) | 🇸🇰 [Slovenčina](README-sk.md) | 🇩🇪 [Deutsch](README-de.md) | 🇭🇺 [Magyar](README-hu.md) | 🇮🇹 [Italiano](README-it.md) | 🇪🇸 [Español](README-es.md)

[![Far Manager](https://img.shields.io/badge/Far%20Manager-3.x-526D82?labelColor=0000FF)](https://www.farmanager.com/) [![Downloads](https://img.shields.io/github/downloads/LanKing/far-tools/AmberModernAndVtrEnabler.farconfig?displayAssetName=false&label=Downloads&color=B87938)](https://github.com/LanKing/far-tools/releases/download/amber-modern-latest/AmberModernAndVtrEnabler.farconfig) [![Version](https://img.shields.io/endpoint?url=https%3A%2F%2Fraw.githubusercontent.com%2FLanKing%2Ffar-tools%2Fmain%2Fthemes%2FAmberModern%2Fversion-badge.json)](https://github.com/LanKing/far-tools/releases) [![License](https://img.shields.io/badge/License-MIT-657A55)](../LICENSE) <img alt="VT required" src="https://img.shields.io/badge/rendering-VT%20required-526D82">

# 🎨 Цветовая тема Amber Modern

> Для тех, кто устал от классической синевы Far Manager и хочет современную, спокойную и приятную для глаз цветовую схему.

![Общий вид](demo.png)

## 📓 Примечания

1. Тема не изменяет правила раскраски файлов — используются уже настроенные в Far Manager правила.
2. Тема задаёт базовые цвета редактора и просмотрщика, но не включает собственную подсветку синтаксиса. Для редактора я использую [Monokai](https://github.com/joric/monokai) — рекомендую попробовать.

## 📦 Установка

Amber Modern использует расширенную цветовую палитру, поэтому требует включённого **Virtual Terminal Rendering**. Необходимая настройка уже включена в файл темы.

1. [Скачайте тему](https://github.com/LanKing/far-tools/releases/download/amber-modern-latest/AmberModernAndVtrEnabler.farconfig) и положите файл в каталог Far Manager.
2. Полностью закройте все запущенные экземпляры Far Manager.
3. Выполните в командной строке:

   ```bat
   far.exe /import AmberModernAndVtrEnabler.farconfig
   ```

4. Запустите Far Manager снова.

## 📄 Лицензия

Amber Modern распространяется по [лицензии MIT](../LICENSE). Доработки и предложения приветствуются 😊
