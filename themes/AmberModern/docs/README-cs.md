🇺🇸 [English](../README.md) | 🇺🇦 [Українська](README-uk.md) | 🇧🇾 [Беларуская](README-be.md) | <sub>ru</sub> [Русский](README-ru.md) | 🇵🇱 [Polski](README-pl.md) | 🇸🇰 [Slovenčina](README-sk.md) | 🇩🇪 [Deutsch](README-de.md) | 🇭🇺 [Magyar](README-hu.md) | 🇮🇹 [Italiano](README-it.md) | 🇪🇸 [Español](README-es.md)

[![Far Manager](https://img.shields.io/badge/Far%20Manager-3.x-00FFFF?labelColor=0000FF)](https://www.farmanager.com/) [![Downloads](https://img.shields.io/github/downloads/LanKing/far-tools/AmberModernAndVtrEnabler.farconfig?displayAssetName=false&label=Downloads&color=B87938)](https://github.com/LanKing/far-tools/releases/download/amber-modern-latest/AmberModernAndVtrEnabler.farconfig) [![Version](https://img.shields.io/github/v/release/LanKing/far-tools?filter=amber-modern-v%2A&sort=semver&label=Version&color=D6A21E)](https://github.com/LanKing/far-tools/releases) [![License](https://img.shields.io/badge/License-MIT-657A55)](../LICENSE) <img alt="VT required" src="https://img.shields.io/badge/rendering-VT%20required-526D82">

# 🎨 Barevné téma Amber Modern

> Pro všechny, které unavuje klasický modrý vzhled Far Manageru a chtějí moderní, klidné a příjemné barevné schéma.

![Celkový vzhled](demo.png)

## 📓 Poznámky

1. Téma nemění pravidla zvýraznění souborů — používají se pravidla již nastavená ve Far Manageru.
2. Téma nastavuje základní barvy editoru a prohlížeče, ale neobsahuje vlastní zvýraznění syntaxe. Pro editor používám [Monokai](https://github.com/joric/monokai) a doporučuji jej vyzkoušet.

## 📦 Instalace

Amber Modern používá rozšířenou paletu barev, a proto vyžaduje zapnuté **Virtual Terminal Rendering**. Potřebné nastavení je již zapnuto v souboru tématu.

1. [Stáhněte téma](https://github.com/LanKing/far-tools/releases/download/amber-modern-latest/AmberModernAndVtrEnabler.farconfig) a umístěte soubor do adresáře Far Manageru.
2. Úplně zavřete všechny spuštěné instance Far Manageru.
3. Spusťte příkaz:

   ```bat
   far.exe /import AmberModernAndVtrEnabler.farconfig
   ```

4. Znovu spusťte Far Manager.

## 📄 Licence

Amber Modern je šířen pod [licencí MIT](../LICENSE). Vylepšení a návrhy jsou vítány 😊
